  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (code === "auth/operation-not-allowed") {
        setError("Google sign-in is disabled in your Firebase project (cofounderai-4d430). Please enable Google under Firebase Console > Authentication > Sign-in method.");
      } else if (code === "auth/unauthorized-domain") {
        setError(`This domain (${window.location.hostname}) is not authorized in Firebase Console. Go to Firebase Console > Authentication > Settings > Authorized domains and add "${window.location.hostname}".`);
        ...
        setError("Invalid Firebase API Key in configuration.");
      } else if (msg) {
        setError(`Sign-in error (${code || "unknown"}): ${msg}`);
      } else {
        ...
        <div className="w-full lg:w-[420px] shrink-0">
          ...
          >
            <div className="text-center space-y-1.5 pb-2 border-b border-stone-100">
              ...
              <p className="text-xs text-stone-500">Sign in with Google to access your AI Co-Founder workspace.</p>
            </div>

            {error && (
              <div className="text-xs bg-red-50 text-red-600 border border-red-200/60 p-3 rounded-xl text-left font-medium">
                {error}
              </div>
            )}
            ...
            <div className="space-y-3.5">
              ...
              <button
                ...
                disabled={loading}
                className="w-full flex items-center justify-center px-4 py-3.5 bg-[#1c1917] hover:bg-[#2e2a27] text-white font-bold rounded-xl transition-all shadow-md hover:-translate-y-0.5 cursor-pointer disabled:opacity-50 text-xs"
              >
            ...
            <p className="text-[11px] text-stone-400 text-center leading-relaxed">
              Google Auth enables deep personalization, permitting your AI Co-Founder to pre-draft actual emails and manage strategic meetings on your behalf.
            </p>
          </motion.div>
        </div>
      </main>
